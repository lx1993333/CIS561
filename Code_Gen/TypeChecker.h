...
string Method_RExpr_handler(AST::ASTNode* input,Scope* current_scope){   //when position == "left" return name, position == "left" return type; 
	if (input->return_type() == "Ident"){
		AST::Ident* inputs = static_cast<AST::Ident*> (input);
		if (inputs->text_ == "this" | inputs->text_ == "Nothing"){
			return inputs->text_;
		}
		else{
//			cout<<"current_scope name: "<<current_scope->scope_name<<endl;
...
string Method_call_handler(AST::ASTNode* input,Scope* current_scope,string position){    //"left"->return name of the function, "right"->return type of the function.
	...
	string caller_type = Method_RExpr_handler(&(inputs->caller_),current_scope);
	...
	int constructor_fix = 0;
	if (caller_type == "Nothing"){
		constructor_fix = 0;
	}
	for (it = arg_block->stmts_.begin();it != arg_block->stmts_.end();it++){
		...
		if (current_scope->scope_name == "Class"){     //if current_scope is Class, that means we are searching for a class method. Thus need to go inside twice. 
			searching_scp = current_scope->next_scope[0]->find_sub_scope(method_name+"_Method");
		}	
		else{
			if(caller_type != "Nothing"){
				searching_scp = global_scp->search_in_sub_scope(caller_type)->next_scope[0]->find_sub_scope(method_name+"_Method");  //finding the function's block scope 
			}
			else{
				searching_scp = global_scp->search_in_sub_scope(method_name)->next_scope[0];
			}
		} 
//		cout<<searching_scp->scope_name<<endl;
//		cout<<searching_scp->table->content.size()-constructor_fix<<endl;
//		cout<<arg_block->stmts_.size()<<endl;
		if(searching_scp->table->content.size()-constructor_fix != arg_block->stmts_.size()){
			std::cout<<"Type Check Error: The number of inputs of function "<< method_name << "() is not correct!"<<endl;
			exit(1);
		}
		
//		cout<<searching_scp->table->content[input_index+constructor_fix].second<<endl;
//		cout<<input_type<<endl;
		if(searching_scp->table->content[input_index+constructor_fix].second != input_type){         //first check if the type of input_type is same with the name in searching_scp
...
//	}
	...
	//cout<<"caller_name: "<<caller_name<<endl;
	
	if (caller_type == "Nothing"){ //then it must be a constructor
		return method_name;
	}
	
	
	
	 //return the scope of method caller
	searching_scp = global_scp->search_in_sub_scope(caller_type);
	
	//cout<<"search:"<<searching_scp->scope_name<<endl;
	
	
	if (searching_scp->class_table->search_in_table(method_name) == "Type_Check_Error" && method_name != "Obj" ){   // Obj() is fine, sine it is a built-in function
...
//	}
	...
	if (position == "right"){    
			...
			if(method_name == "Obj"){ // Obj() returns an Obj
				...
				return searching_scp->class_table->search_in_table(method_name);
			}
	}
	else{
		return method_name;
...
//	Scope* class_scope = current_scope->previous_scope->previous_scope;  //go to the Class scope
	//cout<<"return type == "<< searchine_scope->class_table->search_in_table(func_name)<<endl;
	if (ret_type != searchine_scope->class_table->search_in_table(func_name)){
...
pair<string,string> If_handler(AST::ASTNode* input,Scope* current_scope,string func_name){
	...
	if (RExpr_handler(&(inputs->cond_),current_scope, "right") != "Boolean"){
...
pair<string,string> Method_handler(AST::ASTNode* input,Scope* current_scope){
	...
	Scope* s1= new Scope(current_scope,inputs->name_.text_+"_Method");
	current_scope->add_sub_scope(s1);
	

	pair<string,string> res;
	current_scope->previous_scope->add_to_class_Method_table(inputs->name_.text_, inputs->type_.text_);
	...
	Scope* s2= new Scope(s1,inputs->name_.text_+"_Method_Block");
	s1->add_sub_scope(s2);
...
Scope* classes_traversal(AST::ASTNode *root){
	...
	s1->add_to_scope("True", "Boolean");
	s1->add_to_scope("False", "Boolean");
